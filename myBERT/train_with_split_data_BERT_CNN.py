...
class BERTCNN(nn.Module):
    def __init__(self, bert_path, num_labels, hidden_size, cnn_out_channels, cnn_kernel_sizes):
        super(BERTCNN, self).__init__()
        self.bert = BertModel.from_pretrained(bert_path)
        self.dropout = nn.Dropout(0.1)
        self.conv_layers = nn.ModuleList([
            nn.Conv1d(hidden_size, cnn_out_channels, kernel_size)
            for kernel_size in cnn_kernel_sizes
        ])
        self.fc = nn.Linear(len(cnn_kernel_sizes) * cnn_out_channels, num_labels)
    
    def forward(self, input_ids, attention_mask):
        outputs = self.bert(input_ids, attention_mask=attention_mask)
        sequence_output = outputs.last_hidden_state
        sequence_output = self.dropout(sequence_output)
        sequence_output = sequence_output.permute(0, 2, 1)  # Reshape for CNN
        conv_outputs = [F.relu(conv(sequence_output)) for conv in self.conv_layers]
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = BERTCNN('bert-base-chinese', 10, 768, 256,[3,4,5]).to(device)
