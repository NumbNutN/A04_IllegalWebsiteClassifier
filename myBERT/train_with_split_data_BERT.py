...
class BERTClassifier(nn.Module):
    def __init__(self, bert_path, num_labels, hidden_size):
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = BERTClassifier('bert-base-chinese', 10, 768).to(device)
model = train(model, train_dataloader, epochs=4, evaluation=False)

#torch.save(model,"/A04/IllegalWebsiteClassifier/myModel/bert_split_5_17.pt")
